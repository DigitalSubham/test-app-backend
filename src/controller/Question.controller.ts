...
export const insertQuestions = async (
  ...
  console.log(
    "insertQuestions called with chapterId:",
    chapterId,
    "questions:",
    questions
  );
    ...
    const upsertedTags = await prisma.$transaction(
      tagNames.map((name) =>
        prisma.chapterTag.upsert({
          where: { chapterId_name: { chapterId: Number(chapterId), name } },
          create: { chapterId: Number(chapterId), name },
          update: {},
        })
      )
    );
...
export const extractQuestionsFromPDF = async (
  ...
  try {
    ...
    // Upsert tags
    const tagNames: string[] = Array.from(
      new Set(extractedQuestions.flatMap((q: any) => q.tags))
    );
    const upsertedTags = await prisma.$transaction(
      tagNames.map((name) =>
        prisma.chapterTag.upsert({
